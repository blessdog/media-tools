> **MOVED FROM `STATUS.md` AT THE REPO ROOT, 2026-08-25.** It was a HAND-TYPED
> status file, which LAW #2 bans by name: *"a status file written by hand is a
> cache of the repository with no invalidation: it drifts silently, and the drift
> is invisible because it reads fluently."* It had drifted a week.
>
> Its content is NARRATIVE — what was tried, what shipped, what it taught — so
> under `three-layers-claim-narrative-status` it belongs in `docs/journal/`, and
> it is preserved here whole. **Nothing was deleted.**
>
> STATUS for this repo is `jobs/wang-meng/STATE.md`, GENERATED by a Stop hook
> from git + the knowledge store. Hand edits to it are destroyed, which is the
> only way a generated file stays generated.
