...
usage:
  crop-region.py --master BIG.png --crop crop.json --out REGION.png
                 [--left N] [--right N] [--up N] [--down N]        (grow mode)
  crop-region.py --master BIG.png --rect x0,y0,x1,y1 --k F --out REGION.png
                                                                   (rect mode)

  --master PATH  the full-resolution source
  --crop PATH    crop.json from locate-crop: the master<->shot transform
  --rect COORDS  cut this MASTER-px rect directly (no prior crop needed)
  --k F          master px per region px, rect mode's scale
...
example:
  ...
  crop-region.py --master scroll.png --rect 0,9596,4613,15923 --k 2.34 \
      --out jobs/x/z1/plate.png
"""
