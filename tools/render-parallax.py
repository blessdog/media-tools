...
usage:
  ...
  --z-step F       extra distance per depth level (default 0.035). Small on
                   purpose: at 0.28 the farthest plane sat 3.5x away and
                   rendered at 28% scale, which is not parallax, it is a
                   diorama. A painting wants a few percent of separation.
...
def main() -> int:
        ...
        for p in planes:
            ...
            if tx or ty:
                ...
                for lx, ly in ((0, 0), (iw, 0), (iw, ih), (0, ih)):
                    wx, wy = ox + lx, oy + ly
                    zc = p["z"] + (wy - yc) * tx + (wx - xc) * ty - camZ
                    if zc <= 0.05:              # corner is behind the camera
                        ok = False
                        break
                    sc = fov * f0 / zc
    ...
    print(json.dumps({
        ...
        "zNear": args.z_near, "zStep": args.z_step, "fill": args.fill,
        "keys": keys,
