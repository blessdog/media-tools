...
def parse_args():
    ...
    parser.add_argument("--end-pos", type=str, default=None,
        help="x,y,z camera end position; overrides --type direction. Magnitude = speed. y<0 climbs.")
    parser.add_argument("--target-end", type=str, default=None,
        help="x,y,z final look-target; default 0,0,100 (straight ahead). y<0 pitches up.")
...
def camera_list(
    ...
    if type == "forward":
        ...
        end_pos = np.array([1, 0, 0])
    if end_pos_override is not None:
        end_pos = np.array(end_pos_override, dtype=np.float64)
    ...
    if target_end_override is not None:
        target_end = np.array(target_end_override, dtype=np.float64)
    target_points = np.linspace(target_start, target_end, num_frames * 2)[:num_frames]

    # Custom translation with no custom target = pure translation (parallel
    # gaze): carry the look target with the camera, like the strafe presets.
    carry_target = (type in ("left", "right")) or \
        (end_pos_override is not None and target_end_override is None)
...
if __name__ == "__main__":
    args = parse_args()
    _parse_vec = lambda s: [float(v) for v in s.split(",")] if s else None
    end_pos_ov = _parse_vec(args.end_pos)
    target_end_ov = _parse_vec(args.target_end)
    ...
    intrinsics, extrinsics = camera_list(
        ...
        end_pos_override=end_pos_ov, target_end_override=target_end_ov
    )
